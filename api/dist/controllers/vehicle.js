...
const id = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    vehicle_1.Vehicle.findOne({ _id: req.query.id })
        .then((data) => {
        res.json((0, vehicle_2.makeVehicleView)(data));
    });
});
const create = (req, res) => {
    vehicle_1.Vehicle.create(Object.assign({ _id: (0, crypto_1.randomUUID)() }, req.body))
        .then((result) => {
        console.log(result);
    })
        .catch((err) => {
...
const filter = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    vehicle_1.Vehicle.find(makeMongooseQuery(query))
        ...
        .limit(parseInt(req.query.limit))
        .then((result) => {
        res.json({ data: (0, vehicle_2.makeVehicleArrayView)(result), count: count ? count : 0 });
        res.end();
    }).catch((err) => {
        console.log(err);
        res.end();
